...
- File: `src/api/formatting.py:2`
- Reviewer: `bug`
- Confidence: 0.86
- Evidence: +    name = user.display_name.strip()
...
**Test Suggestions**
- `test_profile_requires_authenticated_user` in `tests/test_formatting.py`: Regression coverage for: User can be None before display_name access
...
**Test Suggestions**
- `test_profile_requires_authenticated_user` in `tests/test_formatting.py`: Regression coverage for: Missing unauthenticated profile regression test
  - Assert: The changed behavior fails before the fix and passes after it.
